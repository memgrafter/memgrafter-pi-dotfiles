 ...
 * spaces. Lines break on real newlines and on literal two-character `\n`
 * sequences (some models emit those instead of real control characters); the
 * sequence is consumed as the line break, display only — the session file and
 * the model's context keep the raw text. Re-formatted on every render tick, so
 * it formats live while the LLM streams the argument (the Text component wraps
 * ANSI-safe at render time).
 */
...
function createThinkTool(): ToolDefinition {
	return {
		...
		description:
			...
			"`kind` is a tag that names what you are doing: plan, react, diagnose, decide, verify, or name your own. " +
