...
priority: 2
assignee: memgrafter
tags: [modes, extension, frag, on-ice]
---
# flexible-role-agent extension: minimal implementation

Minimal flexible-role-agent extension (extensions/flexible-role-agent.ts). --frag flag + /frag command. Replaces system prompt with stable context builder (flexible-role framing + default role at end), roles delivered as post-history custom messages (System: <role>) only on role change (never busts cache). Roles: coding-agent (default, from pi prompt) + analyst. Cache-bust warning box (Continue/Fork/Cancel) on /frag on|off. Design: mpd-itgq.
...
**2026-08-11T18:46:46Z**

Implemented extensions/flexible-role-agent.ts (--frag flag, /frag command, roles as TS consts).

Delivered:
- Context builder system prompt (flexible-role framing; strip-with-fallback; fully role-neutral).
- Roles delivered as post-history custom messages (System: <role>), injected only on role change.
- /frag on|off|status|help|set <role> + /frag set picker + compaction-style autocomplete.
- Cache-bust warning box (Continue/Fork/Cancel) on on/off transitions; content-based detection via ctx.getSystemPrompt() FRAG_MARKER; Esc/undefined aborts (bug found+fixed in testing).
- Fork flow: sessionManager.fork at leaf; pendingApply applied in session_start("fork").
- Persistence via frag-mode session entry; resume restores enabled+role without re-injecting role.

Tested end-to-end against deepseek/deepseek-v4-flash (all cases):
off-state status/error, warning box Continue/Fork/Cancel/Esc, role swap (cache CH98.5% preserved), mode change (cache busted, warned), first-turn + role-change injection shape identical, unknown role error, --frag startup, resume persistence (no duplicate injection), fork on/off transitions (new session file, original untouched).
